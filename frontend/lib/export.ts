import { fetchContributionSvg } from "./api";
...
export async function exportReportSvg(data: ContributionResponse, filename: string): Promise<void> {
  const svg = await fetchContributionSvg(data);
