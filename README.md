...
- GitHub 用户名查询
- 单次查询可在页面输入 Token，或改为读取 `backend/.env` 中的 Token
...
- 手动导出与自动更新共用同一个 SVG 渲染器，版式和动画保持一致
...
GitHub GraphQL API 必须经过身份认证。页面输入框留空不代表无需 Token，而是表示改为读取后端配置：

1. 可以在页面临时输入 Token；它只随本次请求发送，不写入文件或数据库。
2. 也可以把 Token 配置在后端 `backend/.env` 的 `GITHUB_TOKEN` 中，此时页面无需输入。

自动更新模式是另一条明确隔离的链路：首次配置必须在页面输入具备目标仓库写权限的 Token。只有切换到“自动更新”并点击“保存配置”“仅生成测试”或“立即推送”时，页面中的 Token 才会写入 `automation/.env.local`。这个文件已经被 Git 忽略。后续输入框可以留空并复用已保存 Token；普通“生成图谱”请求仍然只在内存使用页面 Token。
